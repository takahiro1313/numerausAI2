"""数値AI ハンズオン — 正解用（講師リファレンス／答え合わせ）。

演習用（app_ensyu.py）の編集セルを記入済みにしたもの。そのまま「学習」「推論」を押せば動く。
裏方・画面は演習用と同一で、編集セルの初期値だけが異なる。

▼ ローカル確認:  streamlit run app_seikai.py
"""
...
PRED_PATH = os.path.join(os.path.dirname(__file__), "data", "予測結果.csv")
VAL_MONTHS = 3
...
def _do_learn():
    ...
    if err is None and ns.get("model") is None:
        err = NameError("model が作られていません。")
    if err is None and not cols:
        err = NameError("手がかり が空です。")
    if err is not None:
    ...
    save_all_predictions(model, load_data(), cols)
...
def _do_predict():
    ...
    pred = ns.get("pred")
    if pred is None:
        st.session_state.pred_err = "pred に予測結果が入っていません。"
        st.session_state.prediction = None
        return
...
# 🟢 Step2 学習（正解・記入済み）
LEARN_SNIPPET = '''# 🟢 ===== ここを編集 =====
# ① AIに見せる「手がかり」を書く（活動の列名）
手がかり = ["問い合わせ数", "架電数", "面談数", "案件提案数"]
...
pred = model.predict(来月予測用)
# 🟢 ===== 編集はここまで =====
'''

render_app(LEARN_SNIPPET, PREDICT_SNIPPET, mode_label="正解用")
